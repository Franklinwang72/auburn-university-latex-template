...
\begin{description}  % no enumitem keys: the TL2025 tagging
  ...
  \item[Section] an element of $\mathcal{F}(U)$: one piece of data over
        the open set $U$.
  \item[Restriction] the map $\mathcal{F}(U) \to \mathcal{F}(V)$ that
        forgets everything outside $V$.
  \item[Germ] the equivalence class of a section near a point.
  \item[Stalk] the group of all germs at a point.
\end{description}
